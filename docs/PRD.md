...
### 1.2 비전
한국 직장인이 자신의 소득 수준에 맞는 **합리적인 주거비**, **차량 구매 가격**, **적정 저축액**을 손쉽게 파악하고, 건전한 재정 계획을 세울 수 있도록 돕는 서비스
...
|------|------|------|------|
| 월급 | number | ✅ | 세후 월급 (만원) |
| 저축 강도 | radio | ✅ | 최소/권장/적극 |
| 연이자율 | number | ❌ | 예상 이자율 (기본 3.5%) |
| 저축 기간 | select | ❌ | 1~10년 |
...
interface SavingsRecommendation {
  monthlySavings: {
    minimum: number     // 세후월급 * 0.10
    recommended: number // 세후월급 * 0.20
    aggressive: number  // 세후월급 * 0.30
  }
  compoundInterest: {
    totalSavings: number    // 원금 + 이자
    totalInterest: number   // 복리 이자
  ...
  emergencyFund: {
    target: number          // 월 생활비 * 6
    monthsToReach: number   // 목표 달성 기간
  }
}
...
│  │            100만원                     500만원      │  │
│  │                                                     │  │
│  │  저축 강도: [최소 10%] [권장 20%] [적극 30%]        │  │
│  │                                                     │  │
│  │  연 이자율: [3.0%] [3.5%] [4.0%] [4.5%]           │  │
│  │                                                     │  │
│  │  저축 기간: [━━━━━━━●━━━━━━━━━━━━━]   5년          │  │
│  │            1년                          10년        │  │
│  └─────────────────────────────────────────────────────┘  │
...
export const metadata: Metadata = {
  ...
  description: '월급과 연봉에 맞는 적정 월세, 차량 가격, 저축액을 계산해보세요. 한국 직장인을 위한 무료 재정 가이드.',
...
| 버전 | 날짜 | 변경 내용 |
|------|------|-----------|
| 3.0 | 2025-12-10 | 저축 플래너 기능 추가, 복리 시뮬레이션 및 비상금 계산 기능 포함, 프로젝트 구조 업데이트 |
